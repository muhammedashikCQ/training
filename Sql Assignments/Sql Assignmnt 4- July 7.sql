...
-- i) Display the no. of books arrived during the last year a
-- ii) Display the no. of books arrived on every Monday during the last year

CREATE TABLE Books(book_id INT IDENTITY PRIMARY KEY,Book_name VARCHAR(50),date DATE);
...
SELECT @maxValue = MAX(YEAR(date)) FROM Books;
SELECT COUNT(book_name) AS Books_last_year FROM Books WHERE YEAR(date)<@maxValue;
SELECT COUNT(book_name) AS Books_last_year_Mondays FROM Books WHERE YEAR(date)<@maxValue AND DATEPART(weekday,date)=2;
